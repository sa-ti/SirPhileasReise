 ...
 */
public class Fragen_Level2 {

    private List<Fragen_Level2> fragenListe;
     ...
     * @throws SQLException
     */
    public Fragen_Level2(int idCountry) throws SQLException{
        try{
     ...
     * @throws SQLException
     */
    public List<Fragen_Level2> randomFrage()throws SQLException{
        List<Fragen_Level2> threeQuestions = new LinkedList<>();
        for (int i = 0; i < 3; i++) {
            int random = random(fragenListe.size() - 1);
            threeQuestions.add(fragenListe.get(random));
            fragenListe.remove(random);
            System.out.println("Random Fragen: " + threeQuestions);
        }
    ...
    public int getAmountFromDB(int idCountry) throws SQLException {
        ...
        ResultSet rs;
        String query = "SELECT COUNT(*) FROM Fragen_Level2 WHERE Country_idCountry = " + idCountry;
        int numberOfRows = 0;
        try {
    ...
    public Fragen_Level2(String frage, String antwort1, String antwort2, String antwort3, String antwort4, String korrekte_Antwort) {
        this.frage = frage;
        this.antwort1 = antwort1;
        this.antwort2 = antwort2;
        this.antwort3 = antwort3;
     ...
     * @throws SQLException
     */
    public List<Fragen_Level2> getQuestionsFromDB(int idCountry) throws SQLException {
        List<Fragen_Level2> liste = new LinkedList();
        Connection con = controllers.Application.db.getConnection();
        PreparedStatement statement;
        ResultSet rs;
        String query = "";
        //getQuiz
        query = "SELECT * FROM Fragen_Level2 WHERE Country_idCountry =" + idCountry;
        statement = con.prepareStatement(query);
        rs = statement.executeQuery();
        while (rs.next()) {
            Fragen_Level2 frage = new Fragen_Level2(rs.getString("Frage"), rs.getString("Antwort1"),
